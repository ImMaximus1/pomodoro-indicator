...
class Pomodoro_Indicator(GObject.GObject):
    ...
    def read_preferences(self):
        ...
        self.long_break_sound_file = configuration.get('long_break_sound_file')
...
118214486317320563625/posts'))
    ...
    def on_break_end(self, widget):
        if self.pomodoros < self.max_pomodoros:
            ...
            self.frame = 0
            self.pomodoro_start.set_label(_('Start'))
            # Indicate the end of pomodoro set
            icon = os.path.join(comun.ICONDIR, 'pomodoro-indicator-%s-%s.svg' % (self.theme, 'set'))
            self.indicator.set_icon(icon)
            self.notification.update('Pomodoro-Indicator', _('The Pomodoro set ends'), icon)
            self.notification.show()
            if self.play_sounds:
                self.play(self.break_sound_file)
            print('**************************')
...
''')
        ...
        about_dialog.set_authors([
            ...
            'Max Medvedovsky <mmedvedovsky@gmail.com>',
            _('Sound:'),
            'Noah McGrath <https://twitter.com/Noah_McGrath>',
        ])
